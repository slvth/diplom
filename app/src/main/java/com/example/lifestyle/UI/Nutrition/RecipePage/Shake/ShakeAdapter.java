package com.example.lifestyle.UI.Nutrition.RecipePage.Shake;
...
public class ShakeAdapter extends RecyclerView.Adapter<ShakeAdapter.ViewHolder>{
    LayoutInflater layoutInflater;
    List<RecipeModel> shakeList;
    RelativeLayout meal_item_id;
    Long kalL,timeL;
    String img_mealS, nameS,description1,description2,description3, complexity;

    public ShakeAdapter(Context context, List<RecipeModel> shakeList) {
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        RecipeModel shake = shakeList.get(position);


        holder.name.setText(shake.getName());
        holder.kal.setText(shake.getKal()+" ккал");
        holder.time.setText(shake.getTime()+" мин");
        Picasso.get().load(shakeList.get(position).getImg_meal()).into(holder.img_meal);

        meal_item_id.setOnClickListener(view -> {

            img_mealS = shakeList.get(position).getImg_meal();
            kalL = shakeList.get(position).getKal();
            nameS = shakeList.get(position).getName();
            timeL = shakeList.get(position).getTime();

            description1 = shakeList.get(position).getDescription1();
            description2 = shakeList.get(position).getDescription2();
            description3 = shakeList.get(position).getDescription3();
            complexity = shakeList.get(position).getComplexity();
